...
**Update 2026-06-12 — the overlay's *duration* problem is solved (commit `f7985ee`)**: the seconds-long overlay was a separate, self-inflicted issue, not this scroll-jump. Next.js serves back/forward navigation from the client Router Cache *even when stale* (`staleTimes` docs: "This doesn't change back/forward caching behavior") — instant back-nav is the framework default. Our own code defeated it three ways, all added earlier to keep the /lists NEW marker honest: (1) `touchListView` called `revalidatePath('/lists')` on every list mount/hide/unmount **and after every outbox mutation** (`engine.ts`); (2) `router.refresh()` in ItemList/TaskList unmount cleanup fired exactly during the back transition; (3) every item server action called `revalidatePath('/lists/${id}')`. All removed; the NEW marker is now kept honest locally instead (`src/lib/sync/overviewLocal.ts`: non-regressive `seedListsOverview` merge + `touchListViewLocal`). The overlay now lasts only as long as the route swap + one Dexie read (`nav.back_overlay_ms` in `app_logs` measures it; expect p50 < ~150 ms). The underlying scroll-jump beneath the overlay is **still masked, not fixed** — everything below stands.
