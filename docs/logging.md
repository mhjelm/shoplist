...
| `extract.*` (`add_items_failed`, `audio_failed`, `tasks_audio_failed`, `note_audio_failed`, `recipe_failed`, `image_http_error`, `image_parse_failed`) | error | import.ts | recipe/list/audio/task-audio/note-audio extraction failed |
| `unfurl.fetch_failed` | warn | import.ts | scrapbook link-unfurl fetch/parse failed (raw link still saved) |
...
- **Server:** `instrumentation.ts` registers a sink into `log.ts` via `registerServerSink()` on Node startup. `emit()`'s server branch calls it for **all** levels (durable rows aren't subject to the prod `info`/`fallback` console suppression — DB volume is cheap and pruned). The sink uses `next/server`'s `after()` so the insert runs post-response.
  ...
  > **GOTCHA (BUG-002, fixed 2026-06-18):** the registered sink **must** be stored on a process-global (`log.ts` uses `Symbol.for('shoplist.log.serverSink')`), **not** a module-level variable. Next.js bundles `instrumentation.ts` into a **separate module graph** from the app, so a module-local `let serverSink` set during `register()` lives in instrumentation's copy of `log.ts` and is invisible to the app's `emit()` (it reads `null` and silently no-ops). This made server durable logging a no-op from day one while client logging worked. Don't "simplify" it back to a module-local var — `src/lib/log.test.ts` guards this. Note this gotcha is invisible in `next dev` (shared module instances); reproduce with `next start`.
