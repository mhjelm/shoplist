...
export default function ShareImportClient({ importId, items, source, lists, currentUserId }: Props) {
  const [selectedListId, setSelectedListId] = useState<string | null>(lists.length === 1 ? lists[0].id : null)
  ...
  async function handleConfirm() {
    if (!selectedListId || selectedCount === 0 || busy) return
    setError(null)
    setBusy(true)
    const chosen = items.filter((_, idx) => selectedItems[idx])
    const result = await confirmShareImport(importId, selectedListId, chosen)
    if (result?.error) {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <main className="max-w-lg mx-auto px-4 py-6 space-y-5">
        <section>
          ...
          {lists.length === 0 ? (
            <p className="text-sm text-gray-500 dark:text-gray-400">Du har inga listor än. Skapa en först.</p>
          ) : (
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          <button
            ...
            disabled={busy || !selectedListId || selectedCount === 0}
          ...
          >
            {busy ? 'Lägger till…' : `Lägg till ${selectedCount}`}
          </button>
        </div>
      </main>
    </div>
