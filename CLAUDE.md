...
**Instant back-nav: /lists/[id] → /lists paints from local cache** — plan at `PLAN.md`, **executed 2026-06-12** (awaiting prod verification + commit). Removes router-cache purges (revalidatePath in touchListView + items actions), adds `seedListsOverview`/`touchListViewLocal` (non-regressive Dexie merge), logs `nav.back_overlay_ms`. See plan for full scope.

_Prior:_ **Service-worker resume hardening — kill the "blank page on cold wake-up"** — plan at `PLAN.md`, **executed 2026-06-12** (awaiting device verification + commit). Source: incident 2026-06-12 ~18:48 local — PWA reopened after a long in-store suspend showed a blank page + the browser's discarded-tab "sad face" for a long time. Root cause (reproduced at the SW layer): `public/sw.js` `handleNavigate` awaited `fetch(req)` with no timeout — a reconnecting radio makes fetch *hang* (never reject), so the offline `catch` fallback never fired for an uncached URL; and the SW cached HTML navs but **not** `/_next/static/*` chunks, so even a cached shell couldn't hydrate. Fix applied: bounded `Promise.race([fetch, timeout(3s)])` → cached shell, plus cache-first immutable assets (`CACHE` v5→v6); 4 new tests in `tests/sw/navigation-cache.test.ts`; `docs/architecture/pwa.md` updated. Also added `src/app/global-error.tsx` — a self-contained last-resort error boundary (catches render crashes that escape all route boundaries incl. the root layout; logs `ui.global_error`), with `tests/components/GlobalError.test.tsx`. The concurrent `realtime.subscribe_error` (`InvalidJWTToken`) logs were benign/self-healing, not the cause.
