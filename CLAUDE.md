...
_Prior:_ **Fix sharing + share a link as a scrap (Web Share Target)** — plan at `PLAN.md`, **executed 2026-06-15**, migration `0030` applied 2026-06-15. Route branches: image → grocery; link (url field OR a URL found in text via `firstUrlIn`) → link path (stores raw link, **no route-time extraction, no empty-bail**); plain text → grocery. `LinkImportMode` shows **all lists** and the **destination kind decides**: shopping/task → `confirmShareLinkAsRecipe` (extract at confirm + `addItems`), notes → `confirmShareLink` (unfurl → scrap). `shareError` surfaced as dismissible toast (`ShareErrorToast.tsx`). `share.received` log for Bug #3. **Correction (same day):** a first pass forced *every* link to a scrap (notes-only picker) — that silently killed recipe-link sharing; fixed by the destination-kind routing above. Still awaiting PWA reinstall on family phone. See `docs/architecture/share-target.md`.
