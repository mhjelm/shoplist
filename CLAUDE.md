...
- `list_text_size` (`'normal'` | `'large'` | `'x-large'`) — read in `src/app/lists/[id]/page.tsx` and passed as `textSize` to `ItemList`, which scales the item rows only (the rest of the chrome stays at its normal size). `ItemList` maps it to the row text class (`text-sm` / `text-base` / `text-xl`) and thumbnail size (`w-12` / `w-16` / `w-20`). The DB CHECK constraint that gates valid values lives in migration `0021_x_large_text_size.sql` (applied). `ItemList` is the single place that computes these classes — `SortableRow`/`ShoppedRow` just render whatever `itemTextClass`/`thumbSizeClass` they're handed. Store mode renders larger than the chrome (`text-lg`, `w-16`) regardless of the preference, and x-large bumps store mode up another step (`text-2xl`, `w-20`).
...
- **Schema changes** go in a new file under `supabase/migrations/` (do not edit `0001_init.sql`). Next migration number is `0022_`.
