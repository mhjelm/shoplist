...
_(none)_
  ...
  Last completed plan: **Shared-items smoke-test follow-ups: clear-shopped bug + icon visibility** — 2026-05-24. `clearShoppedItems` bug fixed: replaced single `or(and(...))` DELETE (unreliable in Supabase JS v2) with two sequential simple DELETEs — one for the source list's checked rows, one cascading to shared siblings via `.in(shared_group_id)`. Edit-mode delete (`deleteItem`) untouched, verified by new test. Chain-link icon visibility improved: added `.sl-shared-icon` CSS utility class with per-theme colors (gray-500/gray-400/teal/polar-mid/dusk-clay) replacing inline `text-gray-300 dark:text-gray-600`. TDD: 6 new tests in `clearShoppedItems.test.ts`. 442 tests pass; `npm run build` clean.
